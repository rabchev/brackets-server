...
function init(srv) {
    ...
    function resolvePath(filePath) {
        if (filePath.startsWith("/extensions/user")) {
            return path.join(srv.extensionsDir, filePath.substr("/extensions/user".length));
        } else if (filePath.startsWith("/extensions/") || filePath.startsWith("/samples/")) {
            return path.normalize(__dirname + "/../brackets-dist" + filePath);
        } else if (filePath.startsWith(srv.root)) {
            return path.normalize(__dirname + "/../brackets-dist" + filePath.substr(srv.root.length));
        }
