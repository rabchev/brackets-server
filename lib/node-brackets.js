...
function doCommand(id, namespace, command, args, res) {
    ...
    } else if (filePath.indexOf(rootUrl) === 0) {
        args[0] = path.normalize(__dirname + "/../brackets/src" + filePath.substr(rootUrl.length));
    } else {
        ...
        if (req.url === "/" && req.originalUrl[idx] !== "/") {
            ...
            if (!index) {
                fs.readFile(__dirname + "/../brackets/src/index.html", function (err, data) {
                    ...
                    rootUrl = pathname.substr(0, pathname.lastIndexOf("/"));
                    sendIndex(res);
                });
                return;
            }
