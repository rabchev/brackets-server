...
function filterExtensions(inst, options, res) {

    function sendResponse() {
        res.writeHead(200, {
            "Content-Type": "application/json"
        });
        res.end(inst._extsCache.data);
    }
    ...
    if (inst._extsCache && inst._extsCache.date >= Date.now() - inst.cacheMaxAge) {
        return sendResponse();
    }
    ...
    request(options, function (err, _res, body) {
        ...
        if (_res.statusCode !== 200) {
            return sendError(_res.statusCode, body);
        }

        // It would be better if we use SAX parser instead of JSON, extensions list could potentially get very large.
        var bodyObj = JSON.parse(body),
            filtered = {};

        Object.keys(bodyObj).forEach(function (key) {
            if (whitelist[key]) {
                filtered[key] = bodyObj[key];
            }
        });
    ...
    srv.on("request", function (req, res) {
        if (req.url.startsWith(root)) {
            ...
            if (url.startsWith("/proxy/")) {
                ...
                if (reqUrl.startsWith("https://s3.amazonaws.com/extend.brackets/registry.json")) {
                    if (inst.enableWhiteList) {
                        filterExtensions(inst, options, res);
                    }
                } else {
                    // Direct proxy
...
function Server(srv, opts) {
    ...
    this.enableWhiteList = opts.enableWhiteList || false;
    this.cacheMaxAge = opts.cacheMaxAge || 1000 * 60 * 60;
